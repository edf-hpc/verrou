
    Vr_instr_kind res;
    res.containFloatCmp=False;
    res.containFloatModOp=False;
    switch(op){
    case Iop_AddF64:
          res.containFloatModOp= vr_replaceBinFpOpScal (sb, stmt, expr, bcNameConvWithCC(add64F), VR_OP_ADD, VR_PREC_DBL, VR_VEC_SCAL, countOnly);
      break;
    case Iop_SubF64:
          res.containFloatModOp= vr_replaceBinFpOpScal (sb, stmt, expr, bcNameConvWithCC(sub64F), VR_OP_SUB, VR_PREC_DBL, VR_VEC_SCAL, countOnly);
      break;
    case Iop_AddF32:
          res.containFloatModOp= vr_replaceBinFpOpScal (sb, stmt, expr, bcNameWithCC(add32F), VR_OP_ADD, VR_PREC_FLT, VR_VEC_SCAL, countOnly);
      break;
    case Iop_SubF32:
          res.containFloatModOp= vr_replaceBinFpOpScal (sb, stmt, expr, bcNameWithCC(sub32F), VR_OP_SUB, VR_PREC_FLT, VR_VEC_SCAL, countOnly);
      break;
    case Iop_MulF64:
          res.containFloatModOp= vr_replaceBinFpOpScal (sb, stmt, expr, bcNameConv(mul64F), VR_OP_MUL, VR_PREC_DBL, VR_VEC_SCAL, countOnly);
      break;
    case Iop_DivF64:
          res.containFloatModOp= vr_replaceBinFpOpScal (sb, stmt, expr, bcNameConv(div64F), VR_OP_DIV, VR_PREC_DBL, VR_VEC_SCAL, countOnly);
      break;
    case Iop_MulF32:
          res.containFloatModOp= vr_replaceBinFpOpScal (sb, stmt, expr, bcName(mul32F), VR_OP_MUL, VR_PREC_FLT, VR_VEC_SCAL, countOnly);
      break;
    case Iop_DivF32:
          res.containFloatModOp= vr_replaceBinFpOpScal (sb, stmt, expr, bcName(div32F), VR_OP_DIV, VR_PREC_FLT, VR_VEC_SCAL, countOnly);
      break;
    case Iop_Add64F0x2:
          res.containFloatModOp= vr_replaceBinFpOpLLO (sb, stmt, expr, bcNameConvWithCC(add64FLLO), VR_OP_ADD, VR_PREC_DBL, VR_VEC_LLO, countOnly);
      break;
    case Iop_Sub64F0x2:
          res.containFloatModOp= vr_replaceBinFpOpLLO (sb, stmt, expr, bcNameConvWithCC(sub64FLLO), VR_OP_SUB, VR_PREC_DBL, VR_VEC_LLO, countOnly);
      break;
    case Iop_Add32F0x4:
          res.containFloatModOp= vr_replaceBinFpOpLLO (sb, stmt, expr, bcNameWithCC(add32FLLO), VR_OP_ADD, VR_PREC_FLT, VR_VEC_LLO, countOnly);
      break;
    case Iop_Sub32F0x4:
          res.containFloatModOp= vr_replaceBinFpOpLLO (sb, stmt, expr, bcNameWithCC(sub32FLLO), VR_OP_SUB, VR_PREC_FLT, VR_VEC_LLO, countOnly);
      break;
    case Iop_Mul64F0x2:
          res.containFloatModOp= vr_replaceBinFpOpLLO (sb, stmt, expr, bcNameConv(mul64FLLO), VR_OP_MUL, VR_PREC_DBL, VR_VEC_LLO, countOnly);
      break;
    case Iop_Div64F0x2:
          res.containFloatModOp= vr_replaceBinFpOpLLO (sb, stmt, expr, bcNameConv(div64FLLO), VR_OP_DIV, VR_PREC_DBL, VR_VEC_LLO, countOnly);
      break;
    case Iop_Mul32F0x4:
          res.containFloatModOp= vr_replaceBinFpOpLLO (sb, stmt, expr, bcName(mul32FLLO), VR_OP_MUL, VR_PREC_FLT, VR_VEC_LLO, countOnly);
      break;
    case Iop_Div32F0x4:
          res.containFloatModOp= vr_replaceBinFpOpLLO (sb, stmt, expr, bcName(div32FLLO), VR_OP_DIV, VR_PREC_FLT, VR_VEC_LLO, countOnly);
      break;
    case Iop_Add64Fx2:
          res.containFloatModOp= vr_replaceBinFullSSE (sb, stmt, expr, bcNameConvWithCC(add64Fx2), VR_OP_ADD, VR_PREC_DBL, VR_VEC_FULL2, countOnly);
      break;
    case Iop_Sub64Fx2:
          res.containFloatModOp= vr_replaceBinFullSSE (sb, stmt, expr, bcNameConvWithCC(sub64Fx2), VR_OP_SUB, VR_PREC_DBL, VR_VEC_FULL2, countOnly);
      break;
    case Iop_Add64Fx4:
          res.containFloatModOp= vr_replaceBinFullAVX (sb, stmt, expr, bcNameConvWithCC(add64Fx4), VR_OP_ADD, VR_PREC_DBL, VR_VEC_FULL4, countOnly);
      break;
    case Iop_Sub64Fx4:
          res.containFloatModOp= vr_replaceBinFullAVX (sb, stmt, expr, bcNameConvWithCC(sub64Fx4), VR_OP_SUB, VR_PREC_DBL, VR_VEC_FULL4, countOnly);
      break;
    case Iop_Add32Fx4:
          res.containFloatModOp= vr_replaceBinFullSSE (sb, stmt, expr, bcNameWithCC(add32Fx4), VR_OP_ADD, VR_PREC_FLT, VR_VEC_FULL4, countOnly);
      break;
    case Iop_Sub32Fx4:
          res.containFloatModOp= vr_replaceBinFullSSE (sb, stmt, expr, bcNameWithCC(sub32Fx4), VR_OP_SUB, VR_PREC_FLT, VR_VEC_FULL4, countOnly);
      break;
    case Iop_Add32Fx8:
          res.containFloatModOp= vr_replaceBinFullAVX (sb, stmt, expr, bcNameWithCC(add32Fx8), VR_OP_ADD, VR_PREC_FLT, VR_VEC_FULL8, countOnly);
      break;
    case Iop_Sub32Fx8:
          res.containFloatModOp= vr_replaceBinFullAVX (sb, stmt, expr, bcNameWithCC(sub32Fx8), VR_OP_SUB, VR_PREC_FLT, VR_VEC_FULL8, countOnly);
      break;
    case Iop_Mul64Fx2:
          res.containFloatModOp= vr_replaceBinFullSSE (sb, stmt, expr, bcNameConv(mul64Fx2), VR_OP_MUL, VR_PREC_DBL, VR_VEC_FULL2, countOnly);
      break;
    case Iop_Div64Fx2:
          res.containFloatModOp= vr_replaceBinFullSSE (sb, stmt, expr, bcNameConv(div64Fx2), VR_OP_DIV, VR_PREC_DBL, VR_VEC_FULL2, countOnly);
      break;
    case Iop_Mul64Fx4:
          res.containFloatModOp= vr_replaceBinFullAVX (sb, stmt, expr, bcNameConv(mul64Fx4), VR_OP_MUL, VR_PREC_DBL, VR_VEC_FULL4, countOnly);
      break;
    case Iop_Div64Fx4:
          res.containFloatModOp= vr_replaceBinFullAVX (sb, stmt, expr, bcNameConv(div64Fx4), VR_OP_DIV, VR_PREC_DBL, VR_VEC_FULL4, countOnly);
      break;
    case Iop_Mul32Fx4:
          res.containFloatModOp= vr_replaceBinFullSSE (sb, stmt, expr, bcName(mul32Fx4), VR_OP_MUL, VR_PREC_FLT, VR_VEC_FULL4, countOnly);
      break;
    case Iop_Div32Fx4:
          res.containFloatModOp= vr_replaceBinFullSSE (sb, stmt, expr, bcName(div32Fx4), VR_OP_DIV, VR_PREC_FLT, VR_VEC_FULL4, countOnly);
      break;
    case Iop_Mul32Fx8:
          res.containFloatModOp= vr_replaceBinFullAVX (sb, stmt, expr, bcName(mul32Fx8), VR_OP_MUL, VR_PREC_FLT, VR_VEC_FULL8, countOnly);
      break;
    case Iop_Div32Fx8:
          res.containFloatModOp= vr_replaceBinFullAVX (sb, stmt, expr, bcName(div32Fx8), VR_OP_DIV, VR_PREC_FLT, VR_VEC_FULL8, countOnly);
      break;
    case Iop_MAddF32:
#ifndef IGNOREFMA
      if(vr.unfused){
          res.containFloatModOp= vr_replaceFMA (sb, stmt, expr, bcNameWithCCUnfused(madd32F), VR_OP_MADD, VR_PREC_FLT, VR_VEC_UNK, countOnly);
      }else{
          res.containFloatModOp= vr_replaceFMA (sb, stmt, expr, bcNameWithCC(madd32F), VR_OP_MADD, VR_PREC_FLT, VR_VEC_UNK, countOnly);
      }
#else //IGNOREFMA
      vr_countOp (sb, VR_OP_MADD, VR_PREC_FLT, VR_VEC_UNK, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNOREFMA
      break;
    case Iop_MAddF64:
#ifndef IGNOREFMA
      if(vr.unfused){
          res.containFloatModOp= vr_replaceFMA (sb, stmt, expr, bcNameConvWithCCUnfused(madd64F), VR_OP_MADD, VR_PREC_DBL, VR_VEC_UNK, countOnly);
      }else{
          res.containFloatModOp= vr_replaceFMA (sb, stmt, expr, bcNameConvWithCC(madd64F), VR_OP_MADD, VR_PREC_DBL, VR_VEC_UNK, countOnly);
      }
#else //IGNOREFMA
      vr_countOp (sb, VR_OP_MADD, VR_PREC_DBL, VR_VEC_UNK, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNOREFMA
      break;
    case Iop_MSubF32:
#ifndef IGNOREFMA
      if(vr.unfused){
          res.containFloatModOp= vr_replaceFMA (sb, stmt, expr, bcNameWithCCUnfused(msub32F), VR_OP_MSUB, VR_PREC_FLT, VR_VEC_UNK, countOnly);
      }else{
          res.containFloatModOp= vr_replaceFMA (sb, stmt, expr, bcNameWithCC(msub32F), VR_OP_MSUB, VR_PREC_FLT, VR_VEC_UNK, countOnly);
      }
#else //IGNOREFMA
      vr_countOp (sb, VR_OP_MSUB, VR_PREC_FLT, VR_VEC_UNK, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNOREFMA
      break;
    case Iop_MSubF64:
#ifndef IGNOREFMA
      if(vr.unfused){
          res.containFloatModOp= vr_replaceFMA (sb, stmt, expr, bcNameConvWithCCUnfused(msub64F), VR_OP_MSUB, VR_PREC_DBL, VR_VEC_UNK, countOnly);
      }else{
          res.containFloatModOp= vr_replaceFMA (sb, stmt, expr, bcNameConvWithCC(msub64F), VR_OP_MSUB, VR_PREC_DBL, VR_VEC_UNK, countOnly);
      }
#else //IGNOREFMA
      vr_countOp (sb, VR_OP_MSUB, VR_PREC_DBL, VR_VEC_UNK, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNOREFMA
      break;
    case Iop_F64toF32:
          res.containFloatModOp= vr_replaceCast (sb, stmt, expr, bcName(cast64FTo32F), VR_OP_CONV, VR_PREC_DBL_TO_FLT, VR_VEC_UNK, countOnly);
      break;
    case Iop_SqrtF64:
#ifndef IGNORESQRT
          res.containFloatModOp= vr_replaceBinFpOpScal_unary (sb, stmt, expr, bcNameConv(sqrt64F), VR_OP_SQRT, VR_PREC_DBL, VR_VEC_SCAL, countOnly);
#else //IGNORESQRT
      vr_countOp (sb, VR_OP_SQRT, VR_PREC_DBL, VR_VEC_SCAL, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNORESQRT
      break;
    case Iop_Sqrt64F0x2:
#ifndef IGNORESQRT
          res.containFloatModOp= vr_replaceBinFpOpLLO_unary (sb, stmt, expr, bcNameConv(sqrt64FLLO), VR_OP_SQRT, VR_PREC_DBL, VR_VEC_LLO, countOnly);
#else //IGNORESQRT
      vr_countOp (sb, VR_OP_SQRT, VR_PREC_DBL, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNORESQRT
      break;
    case Iop_Sqrt64Fx2:
#ifndef IGNORESQRT
          res.containFloatModOp= vr_replaceBinFullSSE_unary (sb, stmt, expr, bcNameConv(sqrt64Fx2), VR_OP_SQRT, VR_PREC_DBL, VR_VEC_FULL2, countOnly);
#else //IGNORESQRT
      vr_countOp (sb, VR_OP_SQRT, VR_PREC_DBL, VR_VEC_FULL2, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNORESQRT
      break;
    case Iop_Sqrt64Fx4:
#ifndef IGNORESQRT
          res.containFloatModOp= vr_replaceBinFullAVX_unary (sb, stmt, expr, bcNameConv(sqrt64Fx4), VR_OP_SQRT, VR_PREC_DBL, VR_VEC_FULL4, countOnly);
#else //IGNORESQRT
      vr_countOp (sb, VR_OP_SQRT, VR_PREC_DBL, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNORESQRT
      break;
    case Iop_SqrtF32:
#ifndef IGNORESQRT
          res.containFloatModOp= vr_replaceBinFpOpScal_unary (sb, stmt, expr, bcName(sqrt32F), VR_OP_SQRT, VR_PREC_FLT, VR_VEC_SCAL, countOnly);
#else //IGNORESQRT
      vr_countOp (sb, VR_OP_SQRT, VR_PREC_FLT, VR_VEC_SCAL, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNORESQRT
      break;
    case Iop_Sqrt32F0x4:
#ifndef IGNORESQRT
          res.containFloatModOp= vr_replaceBinFpOpLLO_unary (sb, stmt, expr, bcName(sqrt32FLLO), VR_OP_SQRT, VR_PREC_FLT, VR_VEC_LLO, countOnly);
#else //IGNORESQRT
      vr_countOp (sb, VR_OP_SQRT, VR_PREC_FLT, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNORESQRT
      break;
    case Iop_Sqrt32Fx4:
#ifndef IGNORESQRT
          res.containFloatModOp= vr_replaceBinFullSSE_unary (sb, stmt, expr, bcName(sqrt32Fx4), VR_OP_SQRT, VR_PREC_FLT, VR_VEC_FULL4, countOnly);
#else //IGNORESQRT
      vr_countOp (sb, VR_OP_SQRT, VR_PREC_FLT, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNORESQRT
      break;
    case Iop_Sqrt32Fx8:
#ifndef IGNORESQRT
          res.containFloatModOp= vr_replaceBinFullAVX_unary (sb, stmt, expr, bcName(sqrt32Fx8), VR_OP_SQRT, VR_PREC_FLT, VR_VEC_FULL8, countOnly);
#else //IGNORESQRT
      vr_countOp (sb, VR_OP_SQRT, VR_PREC_FLT, VR_VEC_FULL8, False);
      addStmtToIRSB (sb, stmt);
#endif //IGNORESQRT
      break;
    case Iop_F64toI64S:
      vr_countOp (sb, VR_OP_CONV, VR_PREC_DBL_TO_INT, VR_VEC_SCAL, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_F64toI64U:
      vr_countOp (sb, VR_OP_CONV, VR_PREC_DBL_TO_INT, VR_VEC_SCAL, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_F64toI32S:
      vr_countOp (sb, VR_OP_CONV, VR_PREC_DBL_TO_SHT, VR_VEC_SCAL, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_F64toI32U:
      vr_countOp (sb, VR_OP_CONV, VR_PREC_DBL_TO_SHT, VR_VEC_SCAL, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Max32Fx4:
      vr_countOp (sb, VR_OP_MAX, VR_PREC_FLT, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Max32F0x4:
      vr_countOp (sb, VR_OP_MAX, VR_PREC_FLT, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Max64Fx2:
      vr_countOp (sb, VR_OP_MAX, VR_PREC_DBL, VR_VEC_FULL2, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Max64F0x2:
      vr_countOp (sb, VR_OP_MAX, VR_PREC_DBL, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Min32Fx4:
      vr_countOp (sb, VR_OP_MIN, VR_PREC_FLT, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Min32F0x4:
      vr_countOp (sb, VR_OP_MIN, VR_PREC_FLT, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Min64Fx2:
      vr_countOp (sb, VR_OP_MIN, VR_PREC_DBL, VR_VEC_FULL2, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Min64F0x2:
      vr_countOp (sb, VR_OP_MIN, VR_PREC_DBL, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Add32Fx2:
      vr_countOp (sb, VR_OP_ADD, VR_PREC_FLT, VR_VEC_FULL2, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_Sub32Fx2:
      vr_countOp (sb, VR_OP_ADD, VR_PREC_FLT, VR_VEC_FULL2, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_CmpF64:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_DBL, VR_VEC_SCAL, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpF32:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_FLT, VR_VEC_SCAL, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_F32toF64:
      vr_countOp (sb, VR_OP_CONV, VR_PREC_FLT_TO_DBL, VR_VEC_UNK, False);
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_CmpEQ64Fx2:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_DBL, VR_VEC_FULL2, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpLT64Fx2:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_DBL, VR_VEC_FULL2, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpLE64Fx2:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_DBL, VR_VEC_FULL2, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpUN64Fx2:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_DBL, VR_VEC_FULL2, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpEQ64F0x2:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_DBL, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpLT64F0x2:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_DBL, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpLE64F0x2:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_DBL, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpUN64F0x2:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_DBL, VR_VEC_LLO, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpEQ32Fx4:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_FLT, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpLT32Fx4:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_FLT, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpLE32Fx4:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_FLT, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpUN32Fx4:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_FLT, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpGT32Fx4:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_FLT, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_CmpGE32Fx4:
      vr_countOp (sb, VR_OP_CMP, VR_PREC_FLT, VR_VEC_FULL4, False);
      addStmtToIRSB (sb, stmt);
      res.containFloatCmp=True;
      break;
    case Iop_ReinterpF64asI64:
    case Iop_ReinterpI64asF64:
    case Iop_ReinterpF32asI32:
    case Iop_ReinterpI32asF32:
    case Iop_NegF64:
    case Iop_NegF32:
    case Iop_AbsF64:
    case Iop_AbsF32:
    case Iop_Abs64Fx2:
    case Iop_Neg64Fx2:
      addStmtToIRSB (sb, stmt);
      break;
    case Iop_SqrtF128:
    case Iop_Sqrt16Fx8:
    case Iop_AddF64r32:
    case Iop_SubF64r32:
    case Iop_MulF64r32:
    case Iop_DivF64r32:
    case Iop_MAddF64r32:
    case Iop_MSubF64r32:
    case Iop_AddF128:
    case Iop_SubF128:
    case Iop_MulF128:
    case Iop_DivF128:
    case Iop_AtanF64:
    case Iop_Yl2xF64:
    case Iop_Yl2xp1F64:
    case Iop_PRemF64:
    case Iop_PRemC3210F64:
    case Iop_PRem1F64:
    case Iop_PRem1C3210F64:
    case Iop_ScaleF64:
    case Iop_SinF64:
    case Iop_CosF64:
    case Iop_TanF64:
    case Iop_2xm1F64:
    case Iop_RSqrtEst5GoodF64:
    case Iop_Log2_64Fx2:
    case Iop_Scale2_64Fx2:
    case Iop_RecipEst64Fx2:
    case Iop_RecipStep64Fx2:
    case Iop_RSqrtEst64Fx2:
    case Iop_RSqrtStep64Fx2:
    case Iop_RecipStep32Fx4:
    case Iop_RSqrtEst32Fx4:
    case Iop_RSqrtStep32Fx4:
    case Iop_RecipEst32F0x4:
    case Iop_RSqrtEst32F0x4:
    case Iop_Scale2_32Fx4:
    case Iop_Log2_32Fx4:
    case Iop_Exp2_32Fx4:
    case Iop_RSqrtEst32Fx8:
    case Iop_RecipEst32Fx8:
    case Iop_RoundF64toF64_NEAREST:
    case Iop_RoundF64toF64_NegINF:
    case Iop_RoundF64toF64_PosINF:
    case Iop_RoundF64toF64_ZERO:
    case Iop_F32toF16x4:
    case Iop_F16toF32x4:
    case Iop_F16toF64x2:
    case Iop_F128toF64:
    case Iop_F128toF32:
    case Iop_F64toI16S:
    case Iop_CmpF128:
    case Iop_PwMax32Fx4:
    case Iop_PwMin32Fx4:
      if(!countOnly){
          vr_maybe_record_ErrorOp (VR_ERROR_UNCOUNTED, op);
      }
      addStmtToIRSB (sb, stmt);
      break;
    default:
      if(operation_with_float_args(expr) && !countOnly){
         vr_maybe_record_ErrorOp (VR_ERROR_UNKNOWN_FLT_IOP, op);
      };
      addStmtToIRSB (sb, stmt);
      break;
    }

    return res;
