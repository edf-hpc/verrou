...
//   print_information_header(context);
   {
      float factorTab[4]={1. , 1.- FLT_EPSILON, 1.- 2*FLT_EPSILON , 1.- 3*FLT_EPSILON};
      double a=FLT_MIN;
      float af=(float)FLT_MIN;
      for( int i=0 ; i< 128 ; i++){
         printf("%d\n" , i);
         double doubleRes[4];
         float  floatRes[4];
         for(int j=0; j<4; j++){
            interflop_vprec_mul_double( a,(double)factorTab[j], &(doubleRes[j]), context);
            floatRes[j]=af*factorTab[j];

            if( ((double)floatRes[j]) != doubleRes[j]){
               printf("%.17f * %.17f => float: %.17f\tdouble: %.17f\n" , a, factorTab[j],  floatRes[j],  doubleRes[j]);
